...
def test_gptneox_forward():
    ...
    flash_model = (
        DecoderModel.from_pretrained("EleutherAI/pythia-1b", config=config)
        .to("cuda")
        .to(torch.bfloat16)
    )
    ...
    bf16_model = (
        AutoModel.from_pretrained("EleutherAI/pythia-1b").to("cuda").to(torch.bfloat16)
    )
    ...
    model = (
        AutoModel.from_pretrained("EleutherAI/pythia-1b").to("cuda").to(torch.float32)
    )
