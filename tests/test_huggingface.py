...
def test_nomic_bert_hf(model_name):
    ...
    print(
        f"Max diff between contrastors and hf ref: {(sequence_output - sequence_output_ref).abs().max().item()}"
    )
    print(
        f"Mean diff between contrastors and hf ref: {(sequence_output - sequence_output_ref).abs().mean().item()}"
    )
    print(
        f"Max diff between hf and hf bf16: {(sequence_output_hf - sequence_output_ref).abs().max().item()}"
    )
    print(
        f"Mean diff between hf and hf bf16: {(sequence_output_hf - sequence_output_ref).abs().mean().item()}"
    )
