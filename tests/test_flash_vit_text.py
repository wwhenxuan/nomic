...
def test_gptneox_forward(model_name):
    ...
    flash_model = (
        DecoderModel.from_pretrained(model_name, config=config)
        .to("cuda")
        .to(torch.bfloat16)
    )

    inputs = {
        "input_ids": torch.randint(0, config.vocab_size, (4, 77)).to("cuda").long()
    }
    inputs["attention_mask"] = torch.ones_like(inputs["input_ids"]).to("cuda").long()

    bf16_model = (
        AutoModel.from_pretrained(model_name).text_model.to("cuda").to(torch.bfloat16)
    )
    ...
    model = (
        AutoModel.from_pretrained(model_name).text_model.to("cuda").to(torch.float32)
    )
