...
def load_loco_dataset(
    ...
    for row in tqdm(range(len(dataset))):
        corpus["Passage_" + str(row)] = {
            "title": "",
            "text": dataset[row][document_column],
        }
...
def load_pubmed_qa(
    ...
    for row in tqdm(range(len(dataset))):
        ...
        corpus["Passage_" + str(row)] = {
            "title": "",
            "text": (" ").join(dataset.iloc[row][document_column]["contexts"]),
        }
...
def load_tau_scrolls_for_summ_screen_fd_gov_report_qmsum(
    dataset_name: str, split: str, document_column: str, query_column: str, subset=None
):
...
def load_qasper(
    ...
    for row in tqdm(range(len(dataset))):
        if document_column == "full_text":
            passage_text = ""
            paragraphs = [
                " ".join(paragraph_sublist)
                for paragraph_sublist in dataset[row][document_column]["paragraphs"]
            ]
            assert len(dataset[row][document_column]["section_name"]) == len(paragraphs)
            for section_header, paragraph in zip(
                dataset[row][document_column]["section_name"], paragraphs
            ):
...
def load_kilt_dataset(
    ...
    file_path = (
        folder_path
        + subset
        + "_reformatted_full_articles_True_shuffle_sections_True_"
        + split
        + ".tsv"
    )
    ...
    dataset = dataset[
        dataset[document_column].apply(lambda x: not isinstance(x, float))
    ]
        ...
        if subset == "hotpotqa":
            ...
            for wiki_id, passage_sub_id in zip(wiki_ids, range(len(wiki_ids))):
                if wiki_id not in wiki_id_to_passage_id:
                    ...
                    corpus[new_passage_id] = {
                        "title": "",
                        "text": dataset.iloc[row][document_column],
                    }
...
def load_kilt_dataset_for_needle_in_haystack(
    dataset_name: str, split: str, document_column: str, query_column: str, subset=None
):
    if split not in ["train", "validation"]:
        ValueError(f"No data split selected! Subset: {subset}, Split: {split}")
    ...
    file_path = (
        folder_path
        + subset
        + "_reformatted_full_articles_True_shuffle_sections_False_gather_distractor_documents_set_True_distractor_documents_count_100_"
        + split
        + ".tsv"
    )
    # file_path = folder_path + subset + "_reformatted_full_articles_True_shuffle_sections_False_gather_distractor_documents_set_True_distractor_documents_count_9_" + split + ".tsv"
...
def load_long_bench(
    ...
    for dataset in datasets:
        if dataset == "vlsp":
            ...
            new_outputs = [
                loaded_dataset["output"][row][0] for row in range(len(loaded_dataset))
            ]
        ...
        if dataset in ["gov_report", "multi_news"]:  # reformat input for gov_report
            ...
            new_inputs = [
                loaded_dataset["answers"][row][0] for row in range(len(loaded_dataset))
            ]
    ...
    for row in tqdm(range(len(combined_dataset))):
        corpus["Passage_" + str(row)] = {
            "title": "",
            "text": combined_dataset.iloc[row][document_column],
        }
...
def load_tau_scrolls_needle(
    ...
    for row in tqdm(range(len(dataset))):
        ...
        distractor_passages_rows = [
            random.randint(test_set_rows_cutoff, len(dataset) - 1) for _ in range(1)
        ]
        distractor_passages = [
            dataset[row_int][document_column] for row_int in distractor_passages_rows
        ]
        distractor_passages.insert(answer_position, answer_text)
        new_corpus["Passage_" + str(row)] = {
            "title": "",
            "text": (" ").join(distractor_passages),
        }
