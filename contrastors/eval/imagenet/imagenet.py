...
def evaluate_imagenet(
        ...
        for batch in tqdm(dataloader):
            ...
            if return_embeddings:
                ...
                for i in range(img.shape[0]):
                    ...
                    pred = pred.t()
                    correct = (
                        pred.eq(target.view(1, -1).expand_as(pred))
                        .detach()
                        .cpu()
                        .numpy()
                    )
                    feats = {
                        ...
                        "embedding": img_embedding.to(torch.float16)
                        .cpu()
                        .detach()
                        .numpy(),
                        "class": curr_class,
    ...
    return (
        torch.tensor([top1 / n]).to(device),
        torch.tensor([top5 / n]).to(device),
        embeddings,
        text_embeddings,
    )
...
def main():
    val_transforms = image_transform(
        image_size=224,
        mean=OPENAI_IMAGE_DATASET_MEAN,
        std=OPENAI_IMAGE_DATASET_STD,
        is_train=False,
    )
    ...
    model = DualEncoder.from_pretrained(path, config=config).to(
    ...
    atlas.map_data(
        ...
        identifier=f"imagenet-{prefix}-embeddings-with-predicted-classes",
    )
