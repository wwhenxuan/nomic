...
class OpenAI_Encoder:
    ...
    def encode(self, sentences: List[str], batch_size: int, **kwargs) -> np.ndarray:
        sentences = [
            cutoff_long_text_for_embedding_generation(
                sentence, self.encoding, cutoff=self.cutoff
            )
        ...
        for sentence_chunks in tqdm(split_list(sentences, self.encoder_batch_size)):
            try:
                encoded_sentences = self.client.embeddings.create(
                    input=sentence_chunks, model=self.embedding_model
                )
            except:
                time.sleep(30)
                encoded_sentences = self.client.embeddings.create(
            ...
            encoded_sentences = [
                sentence_encoding.embedding
                for sentence_encoding in encoded_sentences.data
            ]
    ...
    def encode_queries(
        ...
        queries = [
            cutoff_long_text_for_embedding_generation(
                query, self.encoding, cutoff=self.cutoff
            )
        ...
        for query_chunks in tqdm(split_list(queries, self.encoder_batch_size)):
            try:
                encoded_queries = self.client.embeddings.create(
            ...
            encoded_queries = [
                query_encoding.embedding for query_encoding in encoded_queries.data
            ]
    ...
    def encode_corpus(
        ...
        passages = [
            cutoff_long_text_for_embedding_generation(
                passage, self.encoding, cutoff=self.cutoff
            )
        ...
        for passage_chunks in tqdm(split_list(passages, self.encoder_batch_size)):
            try:
                encoded_passages = self.client.embeddings.create(
                    input=passage_chunks, model=self.embedding_model
                )
            except:
                time.sleep(30)
                encoded_passages = self.client.embeddings.create(
            ...
            encoded_passages = [
                passage_encoding.embedding for passage_encoding in encoded_passages.data
            ]
    ...
    def encode(self, sentences, batch_size=256, **kwargs):
        ...
        with torch.no_grad():
            for i in range(0, len(sentences), batch_size):
                ...
                outputs = self.model(
                    **encoded.to(device), normalize=normalize, binarize=binarize
                )
                embs = outputs["embedding"].cpu().float().numpy()
                if self.matryoshka_dim:
                    embs = embs[:, : self.matryoshka_dim]
                    if normalize:
                        embs = embs / np.expand_dims(
                            np.linalg.norm(embs, axis=-1), axis=1
                        )
            ...
            for i in range(0, len(sentences), batch_size):
                ...
                if not self.clip_model:
                    ...
                    pooled = self.mean_pooling(
                        outputs, encoded["attention_mask"].to(device)
                    )
                else:
