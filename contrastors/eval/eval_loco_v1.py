...
def load_loco_from_hf(
    ...
    if "qasper" in dataset_choice:
        queries = {
            ...
            if corpus[key.replace("Query", "Passage")]["text"] is not None
...
# Boolean flags
parser.add_argument(
    "--model", type=str, default="m2", choices=model_options, help="Model type"
)
parser.add_argument(
    "--model_name",
    type=str,
...
parser.add_argument(
    "--yaml-file",
    type=str,
    default="yamls/embeddings/m2-bert-80M-32k-retrieval.yaml",
...
# Baselines
parser.add_argument(
    "--perform-BM25-and-reranking-with-BGE",
    action="store_true",
    help="Perform BM25 and reranking with BGE",
)
    ...
    for dataset in total_datasets:
        if type(dataset) == str:
            ...
            document_statistics_row = [
                dataset[0] + "_" + str(dataset[4]) + "_" + str(dataset[3])
            ]
        ...
        passage_lengths, query_lengths = calculate_query_document_lengths(
            corpus, queries
        )
        document_statistics_row += [
        ...
        if not perform_BM25_and_reranking_with_BGE:
            ...
            if save_embeddings:
                np.save(
                    f"{dataset_name}_{args.save_embedding_prefix}_query.npy".replace(
                        "/", "_"
                    ),
                    model.query_embeddings,
                )
                np.save(
                    f"{dataset_name}_{args.save_embedding_prefix}_corpus.npy".replace(
                        "/", "_"
                    ),
