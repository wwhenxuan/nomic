...
class GlueTrainer(BaseTrainer):
    ...
    def get_model(self, config):
        ...
        model_config.problem_type = (
            "regression" if self.is_regression else "single_label_classification"
        )
    ...
    def get_dataloaders(self, config, epoch=0):
        ...
        with self.main_process_first():
            ...
            def preprocess_function(examples):
                ...
                result = self.tokenizer(
                    *texts,
                    padding=padding,
                    max_length=config.model_args.seq_len,
                    truncation=True
                )
        ...
        eval_dataset = processed_datasets[
            "validation_matched" if data_config.task_name == "mnli" else "validation"
        ]
        ...
        self.total_num_steps = int(
            len(train_dataloader) // config.train_args.gradient_accumulation_steps
        )
            ...
            for curr_step, batch in enumerate(val_mm_dataloader):
                ...
                if self.num_processes > 1:
                    if curr_step == len(val_mm_dataloader) - 1:
                        predictions = predictions[
                            : len(val_mm_dataloader.dataset) - samples_seen
                        ]
                        references = references[
                            : len(val_mm_dataloader.dataset) - samples_seen
                        ]
                ...
                self.log(
                    {
                        ...
                        "val_mm_metric": mm_metric.compute(),
                        "epoch": step,
                    }
                )
            else:
