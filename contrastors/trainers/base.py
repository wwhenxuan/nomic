...
class BaseTrainer(metaclass=ABCMeta):
    def __init__(self, config, dtype=torch.float32):
        ...
        self.print(
            f"Using {dist.get_world_size() if dist.is_initialized() else 1} GPUs"
        )
        ...
        if getattr(self.config.model_args, "num_experts", 0) > 0:
            ...
            if isinstance(
                ...
                model = model.module
            num_layers = (
                model.trunk.config.n_layer // self.config.model_args.moe_every_n_layers
            )
            ...
            params_per_expert = (
                sum(
                    p.numel()
                    for p in model.trunk.encoder.layers[1].mlp.experts.mlp.parameters()
                    if p.requires_grad
                )
                / num_experts
            )
            active_params = (
                active_params - num_layers * (num_experts - top_k) * params_per_expert
            )
        ...
        self.scheduler = self.get_scheduler(
            config.train_args, self.optimizer, ds_config
        )
        ...
        if self.deepspeed:
            ...
            self.model = {
                "model": model,
                **{k: v for k, v in self.model.items() if k != "model"},
            }
    ...
    def initialize_deepspeed(self, rank, ds_config):
        ...
        if ds_config["train_micro_batch_size_per_gpu"] == "auto":
            ...
            ds_config["train_micro_batch_size_per_gpu"] = (
                data_args.batch_size // self.num_processes
            )
            ds_config["train_batch_size"] = (
                data_args.batch_size * ds_config["gradient_accumulation_steps"]
            )
    ...
    def get_scheduler(self, config, optimizer, ds_config):
        if (
            hasattr(config, "warmup_steps")
            and getattr(config, "warmup_steps") is not None
        ):
            total_num_steps = self.total_num_steps * config.num_epochs
            warmup_steps = config.warmup_steps

        elif (
            hasattr(config, "warmup_pct") and getattr(config, "warmup_pct") is not None
        ):
            total_num_steps = self.total_num_steps * config.num_epochs
        ...
        scheduler = get_scheduler(
            ...
            num_training_steps=(
                total_num_steps if config.schedule_type != "inverse_sqrt" else None
            ),
        )
    ...
    def load_state(self, input_dir):
        if self.deepspeed:
            ...
            self.print(
                f"Loading optimizer and scheduler state from {input_dir}/optimizer.pt"
            )
            self.optimizer.load_state_dict(torch.load(f"{input_dir}/optimizer.pt"))

            self.print(
                f"Loading optimizer and scheduler state from {input_dir}/scheduler.pt"
            )
            self.scheduler.load_state_dict(torch.load(f"{input_dir}/scheduler.pt"))

        self.print(
            f"Loading random states from {input_dir}/random_states_{self.process_index}.pt"
        )
    ...
    def save_state(self, output_dir):
        ...
        if self.deepspeed:
            ...
            if isinstance(self.dataloaders["train"], StreamingShardDataset):
                ...
                with open(
                    f"{ds_state}/rank_{dist.get_rank()}_processed.json", "r"
                ) as f:
                    processed = json.load(f)

                with open(
                    f"{output_dir}/rank_{dist.get_rank()}_processed.json", "w"
                ) as f:
    ...
    def training_step(
        ...
        with torch.autocast(
            device_type="cuda",
            dtype=self.dtype,
            enabled=(not self.deepspeed and not self.config.train_args.grad_cache),
        ):
        ...
        if not self.deepspeed:
            if (
                step + 1
            ) % gradient_accumulation_steps == 0 or step == total_num_steps - 1:
                optimizer.step()
                scheduler.step()
    ...
    def train(self):
        ...
        for epoch in range(start_epoch, train_args.num_epochs):
            if epoch > 0 and getattr(data_config, "streaming", False) is False:
                ...
                train_dataloader = self.get_dataloaders(temp_config, epoch=epoch)[
                    "train"
                ]
            ...
            progbar = tqdm(
                train_dataloader,
                desc=f"Epoch {epoch}",
                disable=not self.global_rank == 0,
            ...
            with context() as p:
                for step, batch in enumerate(progbar):
                    ...
                    if train_args.wandb:
                        self.log(
                            {k: torch.mean(v).item() for k, v in metrics.items()},
                            step=curr_step,
                        )
                    else:
                        self.print(
                            f"Loss: { {k: torch.mean(v).item() for k, v in metrics.items()} }"
                        )
                        # only print every gradient accumulation steps
                        if (
                            step % gradient_accumulation_steps == 0
                            and step > 0
                            or step == total_training_steps - 1
                        ):
                    ...
                    if val_dataloader is not None:
                        if (
                            ...
                            self.eval_loop(
                                dataloader=val_dataloader, step=curr_step, **model
                            )
                    ...
                    if (
                        step > 0
                        and train_args.save_every > 0
                        and step % train_args.save_every == 0
                    ):
