...
from contrastors.dataset.text_text_loader import (
...
class TextTextTrainer(BaseTrainer):
    def __init__(self, config, dtype):
        ...
        if self.matryoshka_dims:
            self.matryoshka_loss_weights = (
                ...
                if config.train_args.matryoshka_dims
                and config.train_args.matryoshka_loss_weights
    ...
    def get_model(self, config):
        ...
        if model_config.checkpoint is None:
            ...
            if config.train_args.router_aux_loss_coef is not None:
                loaded_config.router_aux_loss_coef = (
                    config.train_args.router_aux_loss_coef
                )
            model = BiEncoder.from_pretrained(
                model_config.checkpoint, config=loaded_config
            )
    ...
    def get_dataloaders(self, config, epoch=0):
        ...
        if data_config.streaming:
            ...
            train_dataloader = DataLoader(
                train_dataset, batch_size=1, collate_fn=collate_fn, num_workers=0
            )
        ...
        nano_beir = NanoBEIREvaluator(
            query_prompts=model_args.query_prefix,
            corpus_prompts=model_args.document_prefix,
    ...
    def _grad_cache_forward_step(self, model, batch, logit_scale, **kwargs):
        ...
        document_inputs = {
            k.replace("document_", ""): v for k, v in batch.items() if "document" in k
        }
    ...
    def _forward_step(
        ...
        matroyshka_loss_weights=None,
        **kwargs,
    ):
        normalize = True if matryoshka_dims is None else False
        dataset_name = batch.pop("dataset_name")
        if (
            self.config.model_args.num_experts > 0
            and self.config.train_args.router_aux_loss_coef > 0
        ):
        ...
        if "negative_input_ids" in batch:
            raise NotImplementedError(
                "Negative sampling not supported for text-text models"
            )
        ...
        if (
            ...
            if (
                query_outputs["router_loss"] is not None
                and document_outputs["router_loss"] is not None
            ):
                # megablocks scales loss already
                aux_loss = (
                    query_outputs["router_loss"] + document_outputs["router_loss"]
                )

                if self.config.train_args.wandb and kwargs.get("step") % 1000 == 0:
                    query_tokens_per_expert = query_outputs["tokens_per_expert"].sum(
                        dim=0
                    )
                    query_expert_pct = (
                        query_tokens_per_expert / query_tokens_per_expert.sum()
                    )

                    document_tokens_per_expert = document_outputs[
                        "tokens_per_expert"
                    ].sum(dim=0)
                    document_expert_pct = (
                        document_tokens_per_expert / document_tokens_per_expert.sum()
                    )

                    table = wandb.Table(
                        columns=[
                            f"expert_{i}" for i in range(query_expert_pct.shape[0])
                        ],
