...
from contrastors.models import (
    BertConfig,
    NomicBertForPreTraining,
    bert_config_to_nomic_config,
)
from contrastors.dataset.multilingual import (
    DistributedIterableMLMDataset,
    EvalDistributedIterableMLMDataset,
)
...
class MMLMTrainer(BaseTrainer):
    ...
    def get_model(self, config):
        ...
        if hf_config.vocab_size != len(self.tokenizer):
            self.print(
                f"Resizing model vocab from {hf_config.vocab_size} to {len(self.tokenizer)}"
            )
        ...
        model = NomicBertForPreTraining.from_pretrained(
    ...
    def get_dataloaders(self, config, epoch=0):
        ...
        collator = DataCollatorForLanguageModeling(
            ...
            mlm=True,
            mlm_probability=data_config.mlm_prob,
        )
        ...
        self.total_training_steps = int(
            self.total_num_steps * config.train_args.gradient_accumulation_steps
        )
    ...
    def eval_loop(self, model, dataloader, step):
        ...
        for batch in tqdm(
            dataloader,
            desc=f"Eval epoch step {step}",
            total=dataloader.dataset.num_batches,
        ):
            with torch.no_grad():
