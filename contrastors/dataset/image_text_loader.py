    ...
    for filesample in data:
        ...
        if (
            ...
            current_sample = dict(
                ...
                file_type=filesample["file_type"],
            )
...
def file_expander(
    data, handler=reraise_exception, select_files=None, rename_files=None
):
...
def filter_no_caption_or_no_image(sample):
    ...
    empty_caption = has_caption and (
        len(sample["txt"].strip()) < 2
        or sample["txt"].strip() == b""
        or sample["txt"].strip() == b'"'
    )
    ...
    if tokenizer:
        ...
        if isinstance(text_samples[0], (dict, BatchEncoding)):
            ...
            for k in text_samples[0]:
                text_inputs[k] = torch.stack(
                    [sample[k].squeeze() for sample in text_samples]
                )
...
def get_wds_image_text_dataset(
    ...
    batch_size = (
        int(args.batch_size // dist.get_world_size())
