...
def calculate_auxiliary_loss(
    ...
    if attention_mask is not None:
        ...
        router_prob_per_expert = torch.sum(
            router_weights * router_per_expert_attention_mask, dim=0
    ...
    if tracker is not None:
        ...
        for k in range(top_k):
            ...
            tpe_data = [
                (f"expert_{i}", tokens_per_expert_k[i]) for i in range(num_experts)
            ]
            tpe_table = wandb.Table(data=tpe_data, columns=["expert", "prob"])
            tracker.log(
                {
                    f"tokens_per_expert_pct_k{k}": wandb.plot.bar(
                        tpe_table, "expert", "prob", title=f"tokens_per_expert_pct_k{k}"
                    )
                },
                step=step,
            )
        ...
        rpe_data = [
            (f"expert_{i}", router_prob_per_expert[i]) for i in range(num_experts)
        ]
        rpe_table = wandb.Table(data=rpe_data, columns=["expert", "prob"])
        tracker.log(
            {
                f"router_prob_per_expert_pct": wandb.plot.bar(
                    rpe_table, "expert", "prob", title="router_prob_per_expert_pct"
                )
            },
            step=step,
        )
...
def clip_loss(
    ...
    if bidirectional:
        ...
        loss = F.cross_entropy(similarity_query_document, labels) + F.cross_entropy(
            similarity_document_query, labels
        )  # * dist.get_world_size()
    else:
        loss = (
    ...
    if tracker is not None:
        ...
        tracker.log(
            {f"accuracy/accuracy_{dataset}": accuracy.detach().cpu().item()}, step=step
        )
...
def accumulate_gradients(model, inputs, cache, rand_states, router_aux_coeff):
    ...
    for inp, grad, state, sync_context in zip(
        inputs, cache, rand_states, sync_contexts
    ):
        with sync_context():
            with state:
                with torch.autocast("cuda", dtype=torch.bfloat16):
...
def grad_cache_loss(
    ...
    for chunk_start in range(0, total_bs, chunk_size):
        query_chunk = {
            k: v[chunk_start : chunk_start + chunk_size] for k, v in t1_inputs.items()
        }
        chunked_queries.append(query_chunk)

        document_chunk = {
            k: v[chunk_start : chunk_start + chunk_size] for k, v in t2_inputs.items()
        }
    ...
    accumulate_gradients(
        ...
        chunked_query_cache,
        query_rand_states,
        router_aux_coeff=router_aux_coeff,
    )
    if tower2.training:
        accumulate_gradients(
            tower2,
            chunked_documents,
            chunked_document_cache,
            doc_rand_states,
            router_aux_coeff=router_aux_coeff,
        )
