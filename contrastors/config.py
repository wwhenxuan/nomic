...
class TrainArgs(BaseModel):
    ...
    def validate_eval_strategy(cls, strategy):
        if strategy not in ["steps", "epochs"]:
            raise ValueError(
                f"Eval strategy {strategy} not found in eval strategy registry"
            )
        return strategy
    ...
    def validate_distill_loss_fn(cls, distill_loss_fn):
        if distill_loss_fn not in ["mse", "kd", "towers", "stella"]:
            raise ValueError(
                f"Distill loss fn {distill_loss_fn} not found in distill loss fn registry"
            )
...
class MLMDataArgs(DataArgs):
    ...
    def validate_mlm(self):
        ...
        if tokenized is not None and val_prob is None:
            raise ValueError(
                "Validation MLM probability must be set if tokenized dataset is set"
            )
