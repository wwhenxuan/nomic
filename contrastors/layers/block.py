    ...
    def __init__(
        ...
        if getattr(config, "num_experts", 0) > 0 and moe:
            ...
            self.dropout1 = nn.Dropout(
                getattr(config, "moe_resid_pdrop", config.resid_pdrop)
            )
        ...
        self.drop_path1 = (
            StochasticDepth(drop_path_rate1, mode="row")
            if drop_path_rate1 > 0.0
            else nn.Identity()
        )
        self.drop_path2 = (
            StochasticDepth(drop_path_rate2, mode="row")
            if drop_path_rate2 > 0.0
            else nn.Identity()
        )
    ...
    def forward(
        ...
        if self.prenorm:
            if not self.fused_dropout_add_ln:
                ...
                if (
                    isinstance(self.drop_path1, nn.Identity)
                    or self.drop_path1.p == 0
            ...
            if not self.fused_dropout_add_ln:
                ...
                if (
                    isinstance(self.drop_path2, nn.Identity)
                    or self.drop_path2.p == 0
                    or not self.training
                ):
                    rowscale2 = None
                else:
            ...
            if not self.fused_dropout_add_ln:
                hidden_states = self.norm1(
                    (self.drop_path1(self.dropout1(attn_outputs)) + hidden_states).to(
                ...
                if (
                    ...
                    rowscale1 = self.drop_path1(
                        torch.ones(
                            attn_outputs.shape[:-1],
                            device=attn_outputs.device,
                            dtype=attn_outputs.dtype,
                        )
                    )
