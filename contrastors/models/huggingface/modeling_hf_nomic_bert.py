...
def state_dict_from_pretrained(
    model_name, safe_serialization=False, device=None, dtype=None
):
...
def remap_bert_state_dict(
    ...
    for d in range(config.num_hidden_layers):
        ...
        if not (last_layer_subset and d == config.num_hidden_layers - 1):
            ...
            state_dict[f"bert.encoder.layers.{d}.attn.Wqkv.bias"] = torch.cat(
                [bq, bk, bv], dim=0
            )
        else:
            state_dict[f"bert.encoder.layers.{d}.attn.Wq.weight"] = Wq
            state_dict[f"bert.encoder.layers.{d}.attn.Wkv.weight"] = torch.cat(
                [Wk, Wv], dim=0
            )
            state_dict[f"bert.encoder.layers.{d}.attn.Wq.bias"] = bq
            state_dict[f"bert.encoder.layers.{d}.attn.Wkv.bias"] = torch.cat(
                [bk, bv], dim=0
            )
        ...
        if not remove_cls_weights:
            ...
            if "cls.predictions.decoder.bias" in state_dict:
                ...
                state_dict["cls.predictions.decoder.bias"] = F.pad(
                    decoder_bias,
                    (0, config.vocab_size - decoder_bias.shape[0]),
                    value=-100.0,
                )
    ...
    if remove_bert:
        ...
        state_dict = OrderedDict(
            (remove_bert_prefix(k), v) for k, v in state_dict.items()
        )
...
class NomicBertPreTrainedModel(PreTrainedModel):
    ...
    def from_pretrained(cls, model_name, config=None, *inputs, **kwargs):
        ...
        if "add_pooling_layer" in kwargs:
            model = cls(
                config, *inputs, add_pooling_layer=kwargs.pop("add_pooling_layer")
            )
        else:
...
class NomicVisionPatchEmbeddings(nn.Module):
    def __init__(
        ...
        self.proj = nn.Linear(
            config.num_channels * patch_size[0] * patch_size[1],
            config.n_embd,
        ...
        self.cls_token = (
            nn.Parameter(torch.zeros(1, 1, config.n_embd))
            if not getattr(config, "no_cls_token", False)
            else None
        )
        if config.learned_pos_embedding:
            ...
            if getattr(config, "use_pos_embed", True):
                self.pos_embed = nn.Parameter(
                    torch.zeros(1, self.num_patches + 1, config.n_embd),
                    requires_grad=False,
                )
                pos_embed = get_2d_sincos_pos_embed(
                    config.n_embd, self.grid_size[0], add_cls_token=True
                )
                self.pos_embed.data.copy_(
                    torch.from_numpy(pos_embed).to(self.pos_embed)
                )
            else:
                self.pos_embed = None
        else:
        ...
        if getattr(config, "register_tokens", 0) > 0:
            self.reg_token = nn.Parameter(
                torch.randn(1, config.register_tokens, config.n_embd) * 0.02
            )
        else:
            self.reg_token = None
...
class NomicBertMLP(nn.Module):
    def __init__(
        ...
        self.activation = (
            nn.GELU(approximate=approximate) if activation == "gelu" else activation
        )
...
class NomicBertDynamicNTKRotaryEmbedding(NomicBertRotaryEmbedding):
    ...
    def _update_cos_sin_cache(self, seqlen, device=None, dtype=None):
        ...
        if seqlen > self.max_position_embeddings:
            base = self.base * (
                (self.rotary_scaling_factor * seqlen / self.max_position_embeddings)
...
class NomicBertAttention(nn.Module):
    ...
    def forward(
        ...
        if scaled_dot_product_attention is not None:
            ...
            attention_scores = (
                torch.matmul(query, key.transpose(-1, -2)) / self.norm_factor
            )
