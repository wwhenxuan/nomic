...
class LogitScale(nn.Module):
    def __init__(self, config):
        ...
        self.logit_scale = nn.Parameter(
            torch.ones([]) * np.log(config.logit_scale),
            requires_grad=config.trainable_logit_scale,
        )
...
class BiEncoder(PreTrainedModel):
    def __init__(self, config):
        ...
        if config.use_fused_kernels:
            ...
            if getattr(config, "nomic_encoder", True) is False:
                ...
                if "dinov2" in config.model_name:
                    ...
                    config.model_name.startswith("timm/")
                    or "eva02" in config.model_name
                ):
                ...
                if config.pretrained:
                    self.trunk = NomicBertModel.from_pretrained(
                        ...
                        router_aux_loss_coef=getattr(
                            config, "router_aux_loss_coef", None
                        ),
                        moe_impl=getattr(config, "moe_impl", "megablocks"),
                        ffn_div=getattr(config, "ffn_div", 1.0),
                        moe_normalize_expert_weights=getattr(
                            config, "moe_normalize_expert_weights", False
                        ),
                        expert_choice_router=getattr(
                            config, "expert_choice_router", False
                        ),
                        pad_vocab_size_multiple=getattr(
                            config, "pad_vocab_to_multiple_of", 1
                        ),
                    ...
                    model_config.router_aux_loss_coef = getattr(
                        config, "router_aux_loss_coef", None
                    )
                    model_config.moe_impl = getattr(config, "moe_impl", "megablocks")
                    model_config.moe_normalize_expert_weights = getattr(
                        config, "moe_normalize_expert_weights", False
                    )
                    model_config.expert_choice_router = getattr(
                        config, "expert_choice_router", False
                    )
                    self.trunk = NomicBertModel(
                        config=model_config, add_pooling_layer=False
                    )
        else:
            self.trunk = AutoModel.from_pretrained(
                config.model_name, trust_remote_code=True, add_pooling_layer=False
            )
        ...
        if config.hamming:
            self.hamming = nn.LayerNorm(
                self.trunk.config.hidden_size, elementwise_affine=False
            )
        else:
