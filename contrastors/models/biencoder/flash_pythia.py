...
# try: from flash_attn.ops.rms_norm import RMSNorm
...
class GPTNeoXAttention(nn.Module):
    ...
    def forward(
        ...
        # assuming qkv are all the same length
        ...
        query = query.reshape(bsz, q_len, self.num_attention_heads, self.head_size)
        key = key.reshape(bsz, q_len, self.num_attention_heads, self.head_size)
        value = value.reshape(bsz, q_len, self.num_attention_heads, self.head_size)
...
class GPTNeoXRotaryEmbedding(torch.nn.Module):
    ...
    def forward(self, x, seq_len=None):
        ...
        if seq_len > self.max_seq_len_cached or x.dtype != self.cos_cached.dtype:
...
class GPTNeoXLinearScalingRotaryEmbedding(GPTNeoXRotaryEmbedding):
    ...
    def __init__(
        self, dim, max_position_embeddings, base=10000, device=None, scaling_factor=1.0
    ):
