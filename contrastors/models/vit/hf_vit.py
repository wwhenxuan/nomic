...
def remap_state_dict_hf_vit(state_dict, config):
    ...
    state_dict = OrderedDict(
        (remove_vision_prefix(k), v) for k, v in state_dict.items()
    )
    ...
    def key_mapping_embeddings(key):
        key = re.sub(
            r"^embeddings.position_embeddings", r"embeddings.pos_embed.weight", key
        )
        key = re.sub(
...
def inverse_remap_state_dict_hf_vit(state_dict, config):
    ...
    state_dict = OrderedDict(
        (key_mapping_reverse_ffn(k), v) for k, v in state_dict.items()
    )

    def key_mapping_reverse_attn(key):
        key = re.sub(
            r"layers.(\d+).attn.out_proj", r"layers.\1.attention.output.dense", key
        )
        return key

    state_dict = OrderedDict(
        (key_mapping_reverse_attn(k), v) for k, v in state_dict.items()
    )
    ...
    state_dict = OrderedDict(
        (layer_norm_reverse_mapping(k), v) for k, v in state_dict.items()
    )
    ...
    state_dict = OrderedDict(
        (attention_reverse_mapping_layers(k), v) for k, v in state_dict.items()
    )

    def key_mapping_embeddings_reverse(key):
        key = re.sub(
            r"embeddings.pos_embed.weight",
            r"embeddings.position_embeddings.weight",
            key,
        )
        key = re.sub(
            r"embeddings.proj.", r"embeddings.patch_embeddings.projection.", key
        )
        key = re.sub(r"embeddings.cls_token", r"embeddings.class_embedding", key)
        return key

    state_dict = OrderedDict(
        (key_mapping_embeddings_reverse(k), v) for k, v in state_dict.items()
    )
    ...
    state_dict = OrderedDict(
        (key_mapping_rename_reverse(k), v) for k, v in state_dict.items()
    )

    # Reverse post-processing of embeddings
    state_dict["embeddings.cls_token"] = state_dict.pop("embeddings.class_embedding")
    state_dict["embeddings.position_embeddings"] = state_dict.pop(
