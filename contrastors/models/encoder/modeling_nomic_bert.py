...
class NomicBertPreTrainedModel(PreTrainedModel):
    ...
    def from_pretrained(cls, model_name, config=None, *inputs, **kwargs):
        ...
        remove_bert_prefix = (
            cls != NomicBertForPreTraining and cls != NomicBertForSequenceClassification
        )
        ...
        if pad_vocab_size_multiple := kwargs.pop(
            "pad_vocab_size_multiple", None
        ) or getattr(config, "pad_vocab_size_multiple", None):
        ...
        if kwargs.get("num_experts", 0) > 0:
            ...
            config.moe_normalize_expert_weights = kwargs.pop(
        ...
        if os.path.exists(model_name):
            ...
            if getattr(config, "num_experts", 0) > 0:
                ...
                # and rename to encoder.layers.{i}.mlp.experts.mlp.w1
                    ...
                    for layer_num in range(config.n_layer):
                        if (
                            config.moe_every_n_layers > 1
                            and layer_num % config.moe_every_n_layers == 0
                        ):
                            continue
                        ...
                        mlp_layers = [
                            ...
                            if f"encoder.layers.{layer_num}.mlp" in layer
                        ]
                        for layer_name in mlp_layers:
                            ...
                            if shared_experts > 0:
                                ...
                                if "fc12" in layer_name:
                                    new_name = layer_name.replace(
                                        "fc12.weight", "shared_expert.gate_proj.weight"
                                    )
                                    if remainder > 0:
                                        mean_w1 = expert_layer.view(
                                            remainder, config.n_inner // remainder, -1
                                    ...
                                    new_name = layer_name.replace(
                                        "fc11.weight", "shared_expert.up_proj.weight"
                                    )
                                    if remainder > 0:
                                        mean_v1 = expert_layer.view(
                                            remainder, config.n_inner // remainder, -1
                                    ...
                                    new_name = layer_name.replace(
                                        "fc2.weight", "shared_expert.down_proj.weight"
                                    )
                                    if remainder > 0:
                                        mean_w2 = expert_layer.view(
                                            remainder, config.n_inner // remainder, -1
            ...
            if getattr(config, "num_experts", 0) > 0:
                ...
                if moe_impl == "contrastors":
                    bias_keys = [
                        k for k in model.state_dict().keys() if "mlp.bias" in k
                    ]
                else:
                    bias_keys = [
                        k for k in model.state_dict().keys() if "experts.bias" in k
                    ]

                all_missing = set(router_keys + bias_keys)
                assert (
                    set(load_return.missing_keys) - all_missing == set()
                ), f"Missing keys: {set(load_return.missing_keys) - all_missing}"
...
class NomicBertEncoder(NomicBertPreTrainedModel):
    ...
    def forward(
        ...
        if (
            ...
            and getattr(self.config, "moe_every_n_layers", 0) <= 0
        ):
        ...
        if not self.fused_dropout_add_ln:
            ...
            if isinstance(self.layer_norm, RMSNorm):
                hidden_states = rms_norm(
                    hidden_states, self.layer_norm.weight, self.layer_norm.eps
                )
                ...
                raise ValueError(
                    f"Unsupported layer norm class: {self.layer_norm.__class__.__name__}"
                )
...
class NomicBertLMPredictionHead(nn.Module):
    def __init__(self, config):
        ...
        self.decoder = linear_cls(
            config.n_embd, config.vocab_size, bias=config.mlp_fc1_bias
        )
...
class NomicBertModel(NomicBertPreTrainedModel):
    ...
    def forward(
        ...
        sequence_output, router_outputs = self.encoder(
        ...
        if (
            not router_outputs
            and getattr(self.config, "num_experts", 0) > 0
            and self.training
            and self.config.router_aux_loss_coef > 0
        ):
            if self.config.expert_choice_router:
                ...
                router_loss, tokens_per_expert = moe.batched_load_balancing_loss(
                    megablocks_args
                )
