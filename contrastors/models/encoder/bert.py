...
def remap_bert_state_dict(
    ...
    state_dict = OrderedDict(
        (remap_roberta_prefix(k), v) for k, v in state_dict.items()
    )
    ...
    def key_mapping_decoder_bias(key):
        ...
        key = re.sub(
            r"^bert.lm_head.dense.(weight|bias)",
            r"cls.predictions.transform.dense.\1",
            key,
        )
        key = re.sub(
            r"^bert.lm_head.layer_norm.(weight|bias)",
            r"cls.predictions.transform.layer_norm.\1",
            key,
        )
        key = re.sub(
            r"^bert.lm_head.decoder.weight", "cls.predictions.decoder.weight", key
        )
